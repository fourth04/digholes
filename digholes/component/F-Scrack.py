...
PASSWORD_DIC = [
    '123456', 'admin', 'root', 'password', '123123', '123', '1', '{user}',
...
REGEX = [
    ['ftp', '21', '^220.*?ftp|^220-|^220 Service|^220 FileZilla'],
    ['telnet', '23', '^\\xff[\\xfa-\\xfe]|^\\x54\\x65\\x6c|Telnet'],
    ['mssql', '1433', ''],
    ['mysql', '3306', '^.\\0\\0\\0.*?mysql|^.\\0\\0\\0\\n|.*?MariaDB server'],
    ['postgresql', '5432', ''],
    ['redis', '6379', '-ERR|^\\$\\d+\\r\\nredis_version'],
    ['elasticsearch', '9200', ''],
    ['memcached', '11211', '^ERROR'],
    ['mongodb', '27017', '']]


class Crack():
    """密码破解类，针对不同的port+server，调用不同的破解方法"""
    ...
    def postgresql(self, user, pass_):  # author:hos@YSRC
        ...
        try:
            sock.connect((self.ip, self.port))
            packet_length = len(
                user) + 7 + len("\x03user  database postgres application_name psql client_encoding UTF8  ")
    ...
    def get_auth_data(self, user, password, scramble, plugin):
        ...
        if plugin:
            data += binascii.b2a_hex(plugin) + "0055035f6f73076f737831302e380c5f636c69656e745f6e616d65086c69626d7973716c045f7069640539323330360f5f636c69656e745f76657273696f6e06352e362e3231095f706c6174666f726d067838365f3634"
...
class Nscan:
    """ICMP包测试，包括Ping测(通过SendPingThr对象实现)"""
...
class ThreadNum(threading.Thread):
    """
    功能整合类，负责启用多线程，并且整合各种功能
    主要逻辑就是：
    - 新建一个任务队列，并遍历任务队列
        - 如果是扫描端口类型的任务：
            - 尝试连接端口
            - 根据端口返回内容解析端口所启用服务，如果正确解析出服务，则生成一个弱密码检测类型的任务放入队列中
        - 弱密码类型的任务：
            弱密码检测
    """
                ...
                if task_type == 'portscan':
                    ...
                    if data:
                        ...
                        if server_name:
                            ...
                            queue.put(
                                ":".join([server_name, task_host, task_port]))
                #  如果是除了portscan类型的任务，则尝试弱密码检测
                else:
                    result = pass_crack(task_type, task_host, task_port)
                    if result and result != 3:
                        log(task_type, task_host, task_port, result)
            except Exception as e:
                continue


def scan_port(host, port):
    """负责扫描host的port，主要方式是通过socket连接对应端口，并接受socket信息
    :host: 主机ip地址
    :port: 主机端口号
    :returns: 'NULL', 未连接成功或者接收数据异常
...
def log(scan_type, host, port, info=''):
    """
    根据scan_type的不同，产生不同的log，主要是参数个数不同的问题
    """
    mutex.acquire()
...
def server_discern(host, port, data):
    """根据host，port，以及经过scan_port处理之后，连接socket接收到了data，结合接收到的data和port来分析此端口所起用的服务

    :host: 主机IP
    :port: 主机端口号
    :data: 通过socket连接端口号后的接收数据
    :returns: 服务名
    ...
    for mark_info in REGEX:
        try:
            ...
            if reg and data != 'NULL':
                matchObj = re.search(reg.encode(), data, re.I | re.M)
                if matchObj:
                    return name
